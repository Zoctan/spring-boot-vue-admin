...
function hasPermission(authList, route) {
  if (route.meta && route.meta.auth) {
    return authList.some(auth => route.meta.auth.indexOf(auth) >= 0)
  } else {
    return true
  }
}
 ...
 * @param authList
 */
function filterAsyncRouter(asyncRouterMap, authList) {
    ...
    GenerateRoutes({ commit }, user) {
      return new Promise(resolve => {
        ...
        if (role === 'ROLE_ADMIN') {
          ...
          accessedRouters = filterAsyncRouter(asyncRouterMap, authList)
        }
        // 执行设置路由的方法
